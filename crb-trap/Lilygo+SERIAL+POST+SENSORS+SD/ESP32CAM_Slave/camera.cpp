...
#include "serialtransfer_protocol.h" // for error codes
#include <Arduino.h>
...
bool captureAndSendPhoto(uint16_t lux, uint16_t width, uint16_t height, uint8_t quality, bool saveToDisk) {
  ...
  for (int attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    if (!temp) {
      ...
      delay(1000);
      yield(); // Feed watchdog during retry wait
      continue;
    }
  ...
  // Optionally save photo to SD card before transmission
  if (saveToDisk) {
    if (isSDAvailable()) {
  ...
  // Send photo via SerialTransfer to master
  extern bool sendPhotoViaSerialTransfer(const uint8_t *buffer, uint32_t size, uint8_t quality);
  bool success = sendPhotoViaSerialTransfer(fb->buf, fb->len, quality);

  esp_camera_fb_return(fb);

  if (success) {
    Serial.println("[SERIALTRANSFER] Photo transmission complete");
  } else {
    Serial.println("[SERIALTRANSFER] Photo transmission failed");
  }
