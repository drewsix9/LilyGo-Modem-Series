...
bool initCamera(framesize_t frameSize, uint8_t jpegQuality) {
  ...
  if (s != nullptr) {
    ...
    Serial.println("[CAM] Sensor configuration applied");
  } else {
    Serial.println("[CAM] WARNING: Could not access sensor for configuration");
  }
...
void setHorizontalFlip(bool enabled) {
  ...
  if (s != nullptr) {
    ...
    Serial.printf("[CAM] Horizontal flip: %s\n", enabled ? "ENABLED" : "DISABLED");
  } else {
    Serial.println("[CAM] ERROR: Could not access sensor");
  }
}
