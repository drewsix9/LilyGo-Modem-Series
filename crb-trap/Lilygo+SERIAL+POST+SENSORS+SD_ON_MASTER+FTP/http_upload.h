/**
 * @file      http_upload.h
 * @brief     FTP upload module for posting JPEG photos from RAM via the
 *            A7670 LTE modem.
 *            Handles modem power-on, network registration, and binary upload
 *            using an FTP server.
 * @license   MIT
...
#ifndef FTP_PASS
#define FTP_PASS "rNrKYTX9g7z3RgJRmxWuGHbeu"
#endif

#ifndef FTP_REMOTE_DIR
...
struct UploadMetadata {
  ...
  const char *photoFilename;  // FTP remote filename (EEPROM-based)
};
...
/**
 * @brief  Upload a JPEG image (held in RAM) to the configured FTP server.
 * @param  photoData  Pointer to the JPEG bytes in RAM (e.g. photoBuffer).
 * @param  photoSize  Byte count of the JPEG image.
 * @param  meta       Capture metadata including EEPROM-based filename.
 * @return 201 on success, or -1 on transport/protocol error.
 */
