...
bool receivePhoto() {
  ...
  char nextFilename[32] = {0};
  if (!getNextFilename(nextFilename, sizeof(nextFilename))) {
    strncpy(nextFilename, "IMG_0000.jpg", sizeof(nextFilename) - 1);
  }
  ...
  meta.photoFilename = nextFilename;

  if (initModem()) {
    int uploadCode = uploadPhoto(photoBuffer, photoSize, meta);
    if (uploadCode == 201) {
      Serial.printf("[UPLOAD] FTP upload success: %s\n", nextFilename);
    } else {
      Serial.printf("[UPLOAD] FTP upload failed: code=%d file=%s\n", uploadCode,
                    nextFilename);
    }
  } else {
